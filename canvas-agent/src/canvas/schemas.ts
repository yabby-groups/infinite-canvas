...
export const toolInputSchemas = {
    ...
    canvas_merge_videos: z.object({ nodeIds: z.array(z.string()).min(2), transition: z.enum(["cut", "fade"]).optional(), transitionDurationMs: z.number().min(100).max(5000).optional(), title: z.string().optional() }),
    canvas_inspect_media: z.object({ nodeIds: z.array(z.string()).min(1).optional() }),
    canvas_render_media: z.object({ nodeIds: z.array(z.string()).min(1), videoNodeId: z.string(), audioNodeIds: z.array(z.string()).optional(), trimStartMs: z.number().min(0).optional(), trimEndMs: z.number().min(0).optional(), speed: z.number().min(0.25).max(4).optional(), volume: z.number().min(0).max(4).optional(), mute: z.boolean().optional(), filter: z.enum(["none", "grayscale", "sepia", "contrast"]).optional(), subtitleText: z.string().max(10000).optional(), title: z.string().optional() }),
    canvas_generate_tts: z.object({ text: z.string().min(1), title: z.string().optional() }),
    canvas_transcribe_media: z.object({ nodeId: z.string() }),
