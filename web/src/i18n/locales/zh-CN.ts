export default {
    ...
    agent: {
        ...
        siteTools: { canvasList: "画布列表", generationStatus: "生成任务状态", imageConfig: "生图配置", imageGenerate: "生图工作台生成", videoConfig: "视频配置", videoGenerate: "视频创作台生成", videoMerge: "合并画布视频", mediaInspect: "分析画布媒体", mediaRender: "渲染媒体成片", mediaTts: "生成配音", mediaTranscribe: "转写媒体", promptSearch: "搜索提示词", assetList: "资产列表", assetAdd: "添加资产", unknownTool: "未知工具：{{name}}", canvasLoading: "画布还在加载中，请稍后重试", canvasHint: "用 site_navigate 跳转 /canvas/{id} 打开对应画布", videoMergeCanvasRequired: "请先打开包含视频节点的画布", videoMergeCount: "至少需要选择两个视频节点", videoMergeUnavailable: "所选视频不在本地媒体库中，无法合并", videoMergeFailed: "视频合并失败", mediaCanvasRequired: "请先打开包含本地媒体的画布", mediaUnavailable: "所选媒体不在本地媒体库中", mediaVideoRequired: "请指定一个视频节点", mediaAudioRequired: "请指定一个音频或视频节点", mediaTextRequired: "请提供需要配音的文本", assetsLoading: "资产还在加载中，请稍后重试", assetTitleRequired: "请提供资产标题 title", textContentRequired: "kind=text 时需要提供 content 文本内容", imageUrlRequired: "kind=image 时需要提供 imageUrl（图片地址或 dataURL）", imageReadFailed: "无法读取该图片地址，请改用 dataURL 或可跨域访问的图片链接", assetKindUnsupported: "assets_add 仅支持 kind=text 或 kind=image", imageGenerationStarted: "已跳转生图工作台并触发生成，可用 generation_get_status 查询任务", imageConfigApplied: "已跳转生图工作台并填入参数，未触发生成", videoGenerationStarted: "已跳转视频创作台并触发生成，可用 generation_get_status 查询任务", videoConfigApplied: "已跳转视频创作台并填入参数，未触发生成" },
